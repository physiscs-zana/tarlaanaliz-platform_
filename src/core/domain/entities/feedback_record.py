...
class FeedbackRecord:
    ...
    * KR-019 -- Expert verdict + training_grade.
    ...
    expert_confidence: Decimal | None = None  # numeric(4,3) -> 0-1
    image_quality: Decimal | None = None  # numeric(4,3) -> 0-1
    no_conflict: bool | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.verdict == "corrected" and (not self.corrected_class or not self.corrected_class.strip()):
        ...
        if self.verdict in ("corrected", "rejected") and (not self.notes or not self.notes.strip()):
        ...
        if self.expert_confidence is not None and not (Decimal("0") <= self.expert_confidence <= Decimal("1")):
            raise ValueError("expert_confidence must be between 0 and 1")

        if self.image_quality is not None and not (Decimal("0") <= self.image_quality <= Decimal("1")):
